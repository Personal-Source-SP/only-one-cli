...
- [only-one/rules.md](file:///Users/kiem/Sources/PERSONAL/only-one-cli/only-one/rules.md): Cập nhật negative rules cho `only-one-flash`, anti-concept-duplication, và `only-one-debug` lifecycle isolation.
