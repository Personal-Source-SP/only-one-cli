...
- **[NEVER]** Không diễn giải hoặc mô tả lại cơ chế hoạt động cấp cao trong Section 2 của `plan.md` làm trùng lặp với `concept.md` (Anti-Concept-Duplication). `concept.md` là Single Source of Truth cho cơ chế; Section 2 của `plan.md` chỉ khai báo Type Signatures, Code Contracts và AST Seams.
- **[NEVER]** Không sinh tệp `walkthrough.md` riêng biệt trên đĩa khi kết thúc workflow `/only-one-apply`. Mỗi thư mục task bắt buộc chỉ chứa đúng 2 tệp: `concept.md` và `plan.md` (Strict Two-File Task Invariant); bằng chứng nghiệm thu ghi trực tiếp vào Section 5 của `plan.md` và trình bày qua chat.
...
- **[NEVER]** Không tự ý tạo thư mục task hoặc sinh các tài liệu markdown trên đĩa khi chạy workflow `/only-one-flash`. Toàn bộ plan phải được in ra chat cực ngắn gọn (gồm Mô tả tách bullet points khi có nhiều ý, Target Structure dạng sơ đồ cây ASCII, và Verification) để duy trì tốc độ và Zero Disk Footprint.
