...
import { selectEditorsStep, syncSettingsStep } from './actions/index.js';
...
export const createSettingVsCommand = (deps: ProgramDeps): Command => {
    ...
    cmd.action(async (options: SettingVsCommandOptions) => {
        const editorIds = await selectEditorsStep(deps, options);
        await syncSettingsStep(deps, editorIds, options);
    });
